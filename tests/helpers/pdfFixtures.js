export function createMinimalPdf(content = 'Test PDF') {
  return Buffer.from(
    `%PDF-1.4
1 0 obj<<>>endobj
2 0 obj<< /Length ${content.length + 20} >>stream
BT /F1 12 Tf 100 700 Td (${content}) Tj ET
endstream endobj
3 0 obj<< /Type /Catalog /Pages 4 0 R >>endobj
4 0 obj<< /Type /Pages /Kids [5 0 R] /Count 1 >>endobj
5 0 obj<< /Type /Page /Parent 4 0 R /MediaBox [0 0 612 792] /Contents 2 0 R >>endobj
xref
0 6
0000000000 65535 f 
trailer<< /Size 6 /Root 3 0 R >>
startxref
0
%%EOF`,
  );
}

export function createInvalidPdfBuffer() {
  return Buffer.from('NOT A PDF FILE');
}

export function createCorruptedPdfBuffer() {
  return Buffer.from('%PDF-1.4\ncorrupted without eof');
}
